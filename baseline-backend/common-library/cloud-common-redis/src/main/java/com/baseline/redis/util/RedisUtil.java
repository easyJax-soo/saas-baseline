package com.baseline.redis.util;
...
public class RedisUtil {
    ...
    /**
     * redis基本指令    persist    移除 key 的过期时间，key 将持久保持
     *
     * @param key     键
     * @return 移除的结果
     */
    ...
    /**
     * redis基本指令    randomKey    从当前数据库中随机返回一个 key
     *
     * @return 返回键名
     */
    ...
    /**
     * redis基本指令    renameNx    仅当 newkey 不存在时，将 oldKey 改名为 newkey
     *
     * @param oldKey 旧键名
     * @param newKey 新键名
     * @return 是否设置成功
    ...
    public boolean set(String key, Object val) {
        return set(key, val, DEFAULT_EXPIRE);
    }
     ...
     * @return 追加后的字符数 如果没有这个字符串就新建一个
     */
    public Long incrAndDecr(String key, long addNum) {
        try {
            return redisTemplate.opsForValue().increment(key, addNum);
        } catch (Exception e) {
            e.printStackTrace();
            return 0L;
        }
    ...
    /**
     * Hash基本指令  hIncrByFloat  hash中键的值 + addNum
     *
     * @param key    hash的键
     * @param hKey   需要操作hash中的键
     ...
     * @return 返回增加后的结果
     */
    public Double hIncrByFloat(String key, Object hKey, Double addNum) {
        try {
            return redisTemplate.opsForHash().increment(key, hKey, addNum);
        } catch (Exception e) {
            e.printStackTrace();
...
//    }
    ...
    /**
     * zSet基本指令  ZSCORE 查询value对应的score
     *
     * @param key
     * @param value
     * @return
    ...
    /**
     * zSet基本指令  ZRANK  判断value在zset中的排名
     *
     * @param key
     * @param value
     * @return
     ...
     * @return
     */
    public Set<ZSetOperations.TypedTuple<Object>> zRangeWithScore(String key, int min, int max) {
        try {
    ...
    /**
     * zSet基本指令  ZREVRANGE   查询集合中指定顺序的值
     *
     * 返回有序的集合中，score大的在前面
     *
     * @param key
     ...
     */
    public Distance geoDist(String key, Object member1, Object member2, RedisGeoCommands.DistanceUnit unit) {
        try {
    ...
    /**
     * geospatial 地理位置  基本指令    geoRadius    距离地理位置集合中一个地理位置距离范围内的所有地理位置
     *
     * @param key       地理位置集合的键
     * @param longitude 经度
     * @param latitude  纬度
     ...
     * @return
     */
    public List<GeoResult<RedisGeoCommands.GeoLocation<Object>>> geoRadius(String key
            , double longitude, double latitude, double radius, RedisGeoCommands.DistanceUnit unit){
    ...
    /**
     * geospatial 地理位置  基本指令    geoRadius    距离地理位置集合中一个地理位置距离范围内的有限个地理位置
     *
     * @param key       地理位置集合的键
     * @param longitude 经度
     * @param latitude  纬度
     ...
     */
    public List<GeoResult<RedisGeoCommands.GeoLocation<Object>>> geoRadius(String key
            , double longitude, double latitude, double radius, RedisGeoCommands.DistanceUnit unit, int limit){
    ...
    /**
     * geospatial 地理位置  基本指令    geoRadiusByMember    距离给定一个地点名的地理位置距离范围内的地理位置
     *
     * @param key       地理位置集合的键
     * @param member    圆心点位名称
     * @param radius    半径
     ...
     * @return
     */
    public List<GeoResult<RedisGeoCommands.GeoLocation<Object>>> geoRadiusByMember(String key
            , Object member, double radius, RedisGeoCommands.DistanceUnit unit){
        try {
    ...
    /**
     * BitSet基础操作 GetBit 获取指定偏移量上的位(bit)
     *
     * @param key 需要获取的键
     * @param offset 偏移量
     * @return
