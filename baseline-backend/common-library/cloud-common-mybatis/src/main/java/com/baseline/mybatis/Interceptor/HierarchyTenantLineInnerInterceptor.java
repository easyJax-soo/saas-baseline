...
public class HierarchyTenantLineInnerInterceptor extends TenantLineInnerInterceptor {

    private TenantLineHandler tenantLineHandler;
    
    private HierarchyTenantHandler hierarchyTenantHandler;

    public HierarchyTenantLineInnerInterceptor() {
        super();
    }

    public HierarchyTenantLineInnerInterceptor(TenantLineHandler tenantLineHandler, HierarchyTenantHandler hierarchyTenantHandler) {
        super(tenantLineHandler);
        this.tenantLineHandler = tenantLineHandler;
        this.hierarchyTenantHandler = hierarchyTenantHandler;
    }
     ...
     * 如果已设置，验证该租户ID是否在当前用户的权限范围内（自己或下级租户）
     * @return true表示实体已设置租户ID且有权限操作
     * @throws RuntimeException 如果设置的租户ID不在权限范围内
     */
    ...
    private void validateTenantPermission(Long targetTenantId) {
        ...
        // 获取当前用户及其所有下级租户ID列表
        List<Long> allowedTenantIds = hierarchyTenantHandler.getTenantAndChildrenIds(currentTenantId);
        
        // 检查目标租户ID是否在允许的范围内
        if (!allowedTenantIds.contains(targetTenantId)) {
            String errorMsg = String.format(
                "【租户权限】无权操作租户ID=%d的数据！当前用户租户ID=%d，仅可操作以下租户: %s", 
                targetTenantId, 
                currentTenantId, 
                allowedTenantIds.toString()
            );
            log.error(errorMsg);
    ...
    public Expression buildTableExpression(final Table table, final Expression where, final String whereSegment) {
        ...
        // 捕获Sa-Token上下文未初始化的异常（如CommandLineRunner执行时）
        try {
            ...
            // Sa-Token上下文未初始化（如启动时的CommandLineRunner），跳过租户过滤
            log.debug("【租户拦截】Sa-Token上下文未初始化，跳过租户过滤", e);
            return null;
        } catch (Exception e) {
            // 其他异常也跳过租户过滤，避免启动失败
            log.warn("【租户拦截】获取登录用户异常，跳过租户过滤: {}", e.getMessage());
            return null;
        }
        ...
        // 获取当前租户及其所有子租户ID
        List<Long> tenantIds = hierarchyTenantHandler.getTenantAndChildrenIds(SecurityUtils.getTenantId());
        
        Column tenantColumn = this.getAliasColumn(table);
        
        if (tenantIds.size() == 1) {
            // 只有当前租户，使用等号条件
            return new EqualsTo(tenantColumn, new LongValue(tenantIds.get(0)));
        } else {
            // 有多个租户（包含子租户），使用IN条件
    ...
    /**
     * 重写此方法，在INSERT/UPDATE时检查实体是否已有租户ID
     * 如果实体已设置租户ID（父租户操作子租户数据），则不调用父类方法
     */
    @Override
     ...
     * 层级租户处理器接口
     */
    public interface HierarchyTenantHandler {
        /**
         * 获取租户及其所有子租户ID
         *
