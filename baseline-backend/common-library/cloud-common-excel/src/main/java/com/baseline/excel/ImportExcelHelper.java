...
public class ImportExcelHelper<T> {
     ...
     */
    @SneakyThrows
    public List<T> getList(MultipartFile file, Class<?> clazz, Integer sheetNo, Integer headRowNumber) {
            return getListByStream(file.getInputStream(),clazz,sheetNo,headRowNumber);

    }

    public List<T> getListByStream(InputStream fileStream, Class<?> clazz, Integer sheetNo, Integer headRowNumber) {
        ExcelListener<T> listener = new ExcelListener<>(headRowNumber);
        EasyExcel.read(fileStream, clazz, listener)
